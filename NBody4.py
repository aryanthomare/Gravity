    ...
    for set in itertools.combinations(objs, 2):
        for x in range(steps):
            ...
            if dist < set[0].radius + set[1].radius:
                try:
                    l.append(Planet((rn1x + rn2x)/2,(rn1y + rn2y)/2,(set[0].mass * set[0].vx + set[1].mass * set[1].vx)/(set[0].mass + set[1].mass),(set[0].mass * set[0].vy + set[1].mass * set[1].vy)/(set[0].mass + set[1].mass),set[0].mass + set[1].mass,math.sqrt(set[0].radius**2 + set[1].radius**2),True))
                    l.remove(set[0])
                    l.remove(set[1])
                except:
                    pass
